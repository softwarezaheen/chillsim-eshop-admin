...
const TermsAndConditionsPage = () => {
  ...
  const fetchContent = useCallback(async () => {
    setLoading(true);
    try {
      const { data, error } = await getActiveContent(CONTENT_TYPES.TERMS_AND_CONDITIONS, selectedLanguage);
      if (error) {
        ...
        if (contentData?.content) {
          ...
          if (parsed) {
            ...
            setTitle(editableContent.page_title || 'Terms and Conditions');
            ...
            setTitle(contentData.title || 'Terms and Conditions');
            setIntro('');
  ...
  const fetchHistory = async () => {
    try {
      const { data, error } = await getContentHistory(CONTENT_TYPES.TERMS_AND_CONDITIONS, selectedLanguage);
      if (error) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const { data, error } = await createContentVersion(
        CONTENT_TYPES.TERMS_AND_CONDITIONS,
        selectedLanguage,
        contentData,
        title
      );
  ...
  const loadHistoryVersion = (item) => {
    ...
    if (parsed) {
      // Use the new function to properly unescape content for editing
      const editableContent = prepareContentForEdit(parsed);
      setTitle(editableContent.page_title || item.title || 'Terms and Conditions');
      setIntro(editableContent.page_intro || '');
      setContent(editableContent.page_content || '');
    } else {
      // Fallback to treating as plain text
      setTitle(item.title || 'Terms and Conditions');
      setIntro('');
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
          ...
          <Box>
            <Button
              ...
              sx={{ mr: 2 }}
            >
              {saving ? <CircularProgress size={20} /> : 'Save Terms and Conditions'}
            </Button>
          </Box>
