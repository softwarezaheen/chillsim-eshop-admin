...
const CookiePolicyPage = () => {
  ...
  const fetchContent = useCallback(async () => {
    setLoading(true);
    try {
      const { data, error } = await getActiveContent(CONTENT_TYPES.COOKIES_POLICY, selectedLanguage);
      if (error) {
        ...
        if (contentData?.content) {
          ...
          if (parsed) {
            ...
            setTitle(editableContent.page_title || 'Cookie Policy');
            ...
            setTitle(contentData.title || 'Cookie Policy');
            setIntro('');
  ...
  const fetchHistory = async () => {
    try {
      const { data, error } = await getContentHistory(CONTENT_TYPES.COOKIES_POLICY, selectedLanguage);
      if (error) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        toast.success('Cookie Policy saved successfully');
        fetchContent();
        if (showHistory) {
          fetchHistory();
        }
  ...
  const loadHistoryVersion = (item) => {
    ...
    if (parsed) {
      // Use the new function to properly unescape content for editing
      const editableContent = prepareContentForEdit(parsed);
      setTitle(editableContent.page_title || item.title || 'Cookie Policy');
      setIntro(editableContent.page_intro || '');
      setContent(editableContent.page_content || '');
    } else {
      // Fallback to treating as plain text
      setTitle(item.title || 'Cookie Policy');
      setIntro('');
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
          ...
          <Box>
            <Button
              ...
              sx={{ mr: 2 }}
            >
              {saving ? <CircularProgress size={20} /> : 'Save Cookie Policy'}
            </Button>
          </Box>
