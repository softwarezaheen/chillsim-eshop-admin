...
const AboutUsPage = () => {
  ...
  const fetchContent = useCallback(async () => {
    setLoading(true);
    try {
      const { data, error } = await getActiveContent(CONTENT_TYPES.ABOUT_US, selectedLanguage);
      if (error) {
        ...
        if (contentData?.content) {
          ...
          if (parsed) {
            ...
            setTitle(editableContent.page_title || 'About Us');
            ...
            setTitle(contentData.title || 'About Us');
            setIntro('');
  ...
  const fetchHistory = async () => {
    try {
      const { data, error } = await getContentHistory(CONTENT_TYPES.ABOUT_US, selectedLanguage);
      if (error) {
  ...
  const loadHistoryVersion = (item) => {
    ...
    if (parsed) {
      // Use the new function to properly unescape content for editing
      const editableContent = prepareContentForEdit(parsed);
      setTitle(editableContent.page_title || item.title || 'About Us');
      setIntro(editableContent.page_intro || '');
      setContent(editableContent.page_content || '');
    } else {
      // Fallback to treating as plain text
      setTitle(item.title || 'About Us');
      setIntro('');
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
          ...
          <Box>
            <Button
              ...
              sx={{ mr: 2 }}
            >
              {saving ? <CircularProgress size={20} /> : 'Save About Us'}
            </Button>
          </Box>
